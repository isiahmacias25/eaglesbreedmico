document.addEventListener("DOMContentLoaded", async () => {
  ...
  const { getFirestore, collection, getDocs, doc, getDoc } = await import("https://www.gstatic.com/firebasejs/9.22.2/firebase-firestore.js");
  ...
  monthButtons.forEach(button => {
    button.addEventListener("click", () => {
      ...
      monthTitle.textContent = `${months[parseInt(month, 10) - 1]} 2025`;
  ...
  async function generateCalendar(month, year = new Date().getFullYear()) {
    ...
    const totalDays = getDaysInMonth(year, monthNum - 1);
    ...
    eventsSnap.forEach(docSnap => {
      ...
      if (eventMonth === monthNum && eventYear === year) {
    ...
    for (let day = 1; day <= totalDays; day++) {
      ...
      const style = eventType ? `style="background:${color}; color: white; cursor:pointer;"` : '';
      if (eventType) {
        calendarDiv.innerHTML += `
          <div class="calendar-day" data-day="${day}" data-month="${monthNum}" data-year="${year}" data-type="${eventType}" ${style}>
            ${day}
          </div>`;
      } else {
    ...
    // Add click listeners on event days
  ...
  // Show event modal on view button click
